...
public class PunishmentCommands
{
			...
			if (Commons.bansDatabase.isBanned(Name) == false)
			{
				...
				if (Args.length >= 1)
				{
					...
					if (Bukkit.getPlayer(Name) != null)
					{
						...
						CommonUtils.MessageAllPlayers(new String[] { ChatColor.GREEN + "[Tunnels Network] " + ChatColor.YELLOW + Name + ChatColor.GREEN + " was banned by " + ChatColor.YELLOW + BannedBy, ChatColor.RED + " - Reason: " + Reason, ChatColor.RED + " - Expires: " + (PermBan == true ? "Never" : (TimeHandler.getDurationBreakdown(BanExpires - System.currentTimeMillis()))) });
					}
					else
					{
						...
						{
							Sender.sendMessage(ChatColor.GREEN + "[Tunnels Network] " + ChatColor.YELLOW + bPlayer.getName() + ChatColor.GREEN + " isn't a valid player; They've never played on this server.");
						}
					}
				}
				else
	...
	public void UnbanCommand(CommandSender Sender, String[] Args)
	{
		if (Args.length > 0)
		{
			...
			if (Commons.bansDatabase.Pardon(Unban, Unbanner) == true)
			{
				CommonUtils.MessageAllPlayers(ChatColor.GREEN + "[Tunnels Network] " + ChatColor.YELLOW + Unban + ChatColor.GREEN + " was unbanned by " + ChatColor.YELLOW + Unbanner);
