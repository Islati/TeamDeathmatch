...
public class GameOverRunnable implements Runnable {
	...
	public void run() {
		...
		if (PlayerHandler.getOnlinePlayersCount() <= 0) {
			...
			TDMGame.gameInProgress = false;
			this.gameCurrentTicks = 0;
		...
		if (this.gameCurrentTicks <= this.gameStopTicks) {
		...
		} else if (this.gameCurrentTicks >= this.gameStopTicks) {
			TDMGame.gameInProgress = false;
			this.gameCurrentTicks = 0;
