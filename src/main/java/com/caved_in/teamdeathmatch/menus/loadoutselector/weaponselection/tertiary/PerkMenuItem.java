		...
		if (GamePlayer.hasPerk(this.perk)) {
			...
			if (!hasClicked) {
				...
				if (playerWrapper.getCurrency() >= perkPurchaseCost) {
					...
					player.sendMessage(StringUtil.formatColorCodes("&bYou've purchased the &e" + perk.getPerkName() + "&b for &a" + perk
							.getPurchaseCost() + "; You have &a" + ((int) playerWrapper.getCurrency()) + " &b XP remaining."));
					getMenu().closeMenu(player);
				} else {
					player.sendMessage(StringUtil.formatColorCodes("&cYou don't have enough XP to unlock &e" + perk.getPerkName()));
				}
			}
		}
	}
