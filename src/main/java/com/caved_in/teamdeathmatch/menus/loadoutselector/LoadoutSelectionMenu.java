...
public class LoadoutSelectionMenu {
	...
	private LoadoutSelectionMenu(String playerName) {
		...
		for (int I = 0; I < loadoutLimit; I++) {
			...
			loadoutItem.setDescriptions(Arrays.asList(Items.getItemName(Game.crackShotAPI.generateWeapon(gamePlayer.getPrimaryGunID(loadoutNumber))),
					Items.getItemName(Game.crackShotAPI.generateWeapon(gamePlayer.getSecondaryGunID(loadoutNumber)))));
